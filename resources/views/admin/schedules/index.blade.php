        ...
        <div class="col-md-4 ms-auto">
            <form action="{{ route('admin.schedules.index') }}" method="GET">
                <div class="input-group shadow-sm rounded-3 overflow-hidden border">
                    ...
                    <input type="text" name="search" class="form-control border-0 px-2" placeholder="Cari film atau studio..." value="{{ request('search') }}">
                    @if(request('search'))
                        <a href="{{ route('admin.schedules.index') }}" class="btn btn-light border-0 d-flex align-items-center"><i class="bi bi-x-lg text-muted"></i></a>
                    @endif
                </div>
            </form>
        </div>
