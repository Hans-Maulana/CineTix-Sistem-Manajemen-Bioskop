...
<style>
  ...
  .rfnd-input:focus, .rfnd-textarea:focus {
    outline: none;
    border-color: #1A1953;
    ...
    <div class="col-lg-8">
      ...
      <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item"><a href="{{ route('booking.guest-ticket', ['booking' => $booking, 'token' => $token]) }}" class="text-primary text-decoration-none">E-Ticket</a></li>
      ...
      <div class="rfnd-hero mb-4" data-aos="fade-down">
        <div style="position:relative;z-index:2;">
          <span class="badge bg-warning text-dark rounded-pill mb-2 px-3 py-2" style="font-size:.75rem;">
            <iconify-icon icon="lucide:rotate-ccw" class="me-1"></iconify-icon> Pengajuan Refund (Guest)
          </span>
          <h4 class="text-white fw-bold mb-1">Batalkan & Refund Tiket</h4>
          <p class="text-white-50 mb-0 small">
            Mohon isi formulir dengan benar. Dana akan ditransfer ke rekening yang Anda berikan.
          </p>
        </div>
      </div>
      ...
      <div class="row g-4" data-aos="fade-up">
        ...
        <div class="col-md-7 d-flex flex-column gap-4">
          <div class="rfnd-card flex-grow-1">
            ...
            <div class="rfnd-card-body">
              <form method="POST" action="{{ route('booking.guest-refund.store', ['booking' => $booking, 'token' => $token]) }}" id="refundForm">
                @csrf
                
                {{-- Verifikasi Email --}}
                <div class="mb-4">
                  <label for="guestEmail" class="form-label small fw-bold text-dark">
                    Konfirmasi Alamat Email (Verifikasi) <span class="text-danger">*</span>
                  </label>
                  <input type="email" class="rfnd-input @error('guest_email') is-invalid @enderror" id="guestEmail" name="guest_email" value="{{ old('guest_email') }}" placeholder="Masukkan email yang digunakan saat pesan tiket" required>
                  <div class="form-text mt-1" style="font-size: 0.75rem;"><iconify-icon icon="lucide:shield-check" class="text-success"></iconify-icon> Keamanan: Buktikan bahwa tiket ini milik Anda.</div>
                  @error('guest_email')
                    <div class="invalid-feedback">{{ $message }}</div>
                  @enderror
                </div>

                <hr class="my-4" style="border-color: rgba(26,25,83,.08);">

                <div class="mb-4">
                  <label class="form-label small fw-bold text-dark mb-3">Informasi Rekening Pengembalian</label>
                  
                  <div class="mb-3">
                    <label for="bankName" class="form-label small text-muted mb-1">Bank / E-Wallet <span class="text-danger">*</span></label>
                    <input type="text" class="rfnd-input @error('bank_name') is-invalid @enderror" id="bankName" name="bank_name" value="{{ old('bank_name') }}" placeholder="Contoh: BCA / GoPay / OVO" required>
                  </div>
                  
                  <div class="mb-3">
                    <label for="accountNumber" class="form-label small text-muted mb-1">Nomor Rekening / No. HP <span class="text-danger">*</span></label>
                    <input type="text" class="rfnd-input @error('account_number') is-invalid @enderror" id="accountNumber" name="account_number" value="{{ old('account_number') }}" placeholder="Masukkan nomor" required>
                  </div>

                  <div class="mb-3">
                    <label for="accountName" class="form-label small text-muted mb-1">Atas Nama <span class="text-danger">*</span></label>
                    <input type="text" class="rfnd-input @error('account_name') is-invalid @enderror" id="accountName" name="account_name" value="{{ old('account_name') }}" placeholder="Nama pemilik rekening" required>
                  </div>
                </div>
                ...
                <div class="mb-3">
                  ...
                  <textarea
                    ...
                    rows="3"
                    maxlength="1000"
                    placeholder="Contoh: Tidak bisa hadir karena ada keperluan mendadak..."
                    required
                ...
                <div class="form-check mb-4">
                  ...
                  <label class="form-check-label small text-muted" for="agreePolicy">
                    Saya menyetujui kebijakan potongan administrasi {{ \App\Models\Booking::REFUND_ADMIN_FEE_PERCENT }}% dan memverifikasi kebenaran nomor rekening saya.
                  </label>
                </div>
                ...
                <div class="d-flex flex-column gap-2">
                  ...
                  <a href="{{ route('booking.guest-ticket', ['booking' => $booking, 'token' => $token]) }}" class="btn-rfnd-cancel text-center text-decoration-none">
                    Batalkan, Kembali ke E-Ticket
                  </a>
                </div>
              </form>
            </div>
  ...
  <script>
    ...
    form.addEventListener('submit', function(e) {
      ...
      Swal.fire({
        ...
        text: 'Apakah data rekening sudah benar? Refund akan ditransfer ke rekening tersebut dan tidak dapat dibatalkan.',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#1A1953',
        cancelButtonColor: '#e4e8ef',
        confirmButtonText: 'Ya, Ajukan Refund',
        cancelButtonText: '<span style="color:#5c6478">Periksa Lagi</span>',
        reverseButtons: true,
        customClass: {
          popup: 'rounded-4'
        }
